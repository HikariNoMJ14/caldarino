...
function fgreen_load_scripts() {
	...
	wp_enqueue_script( 'fgreen-js', get_template_directory_uri() . '/js/fgreen.js',
		array( 'jquery', 'viewportchecker' ) );

	$data = array(
		'loading_effect' => ( get_theme_mod('fgreen_animations_display', 1) == 1 ),
	);
	wp_localize_script('fgreen-js', 'fgreen_options', $data);
}
...
function fgreen_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		'fgreen_slider_section',
		array(
			'title'       => __( 'Slider', 'fgreen' ),
	...
	for ($i = 1; $i <= 3; ++$i) {
		...
		$slideContentId = 'fgreen_slide'.$i.'_content';
		$slideImageId = 'fgreen_slide'.$i.'_image';
		$defaultSliderImagePath = get_template_directory_uri().'/images/slides/'.$i.'.jpg';
	
		// Add Slide Content
		$wp_customize->add_setting(
			$slideContentId,
			array(
				'default'           => __( '<h2>This is Default Slide Title</h2><p>You can completely customize Slide Background Image, Title, Text, Link URL and Text.</p><a title="Read more" href="#">Read more</a>', 'fgreen' ),
		...
		$wp_customize->add_control( new WP_Customize_Control( $wp_customize, $slideContentId,
									array(
										'label'          => sprintf( esc_html__( 'Slide #%s Content', 'fgreen' ), $i ),
										'section'        => 'fgreen_slider_section',
		...
		$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, $slideImageId,
				array(
					'label'   	 => sprintf( esc_html__( 'Slide #%s Image', 'fgreen' ), $i ),
					'section' 	 => 'fgreen_slider_section',
	 ...
	 */
	$wp_customize->add_section(
		'fgreen_animations_display',
		array(
			'title'       => __( 'Animations', 'fgreen' ),
			'capability'  => 'edit_theme_options',
		)
	);
	...
	$wp_customize->add_control( new WP_Customize_Control( $wp_customize,
						'fgreen_animations_display',
							array(
								'label'          => __( 'Enable Animations', 'fgreen' ),
								'section'        => 'fgreen_animations_display',
								'settings'       => 'fgreen_animations_display',
								'type'           => 'checkbox',
							)
						)
	);
 ...
 * Displays the slider
 */
function fgreen_display_slider() {
?>
		...
		<ul id="slider">
			<?php
				...
				for ( $i = 1; $i <= 3; ++$i ) {
						...
						$defaultSlideContent = __( '<h3>This is Default Slide Title</h3><p>You can completely customize Slide Background Image, Title, Text, Link URL and Text.</p><a title="Read more" href="#">Read more</a>', 'fgreen' );
						
						$defaultSlideImage = get_template_directory_uri().'/images/slides/' . $i .'.jpg';

						$slideContent = get_theme_mod( 'fgreen_slide'.$i.'_content', html_entity_decode( $defaultSlideContent ) );
						$slideImage = get_theme_mod( 'fgreen_slide'.$i.'_image', $defaultSlideImage );
